...
public class MemberCall {
    ...
    void instanceMethod1() {
        System.out.println(cv);
        System.out.println(iv);     // 인스턴스 메소드에서는 인스턴스 변수를 바로 사용 가능.
    }

    static void staticMethod2() {
        staticMethod1();
        // instanceMethod1();       // 에러, 클래스 메소드에서는 인스턴스 메소드를 호출할 수 없음.

        MemberCall c = new MemberCall();
        c.instanceMethod1();        //인스턴스를 생성한 후 호출할 수 있음.
    }

    void instanceMethod2() {
        staticMethod1();
        instanceMethod1();
        // 인스턴스 메소드, 클래스 메소드 모두 인스턴스 생성 없이 호출이 가능.
    }
}
